...
import { AuthenticationBindings, AuthenticationComponent, AuthenticationConfig, BearerStrategyFactoryProvider, Strategies } from 'loopback4-authentication';
import {
...
import { AuthenticationServiceComponent, AuthServiceBindings, JWTAsymmetricSignerProvider, OtpMethodType, VerifyBindings } from '@sourceloop/authentication-service'
...
export class AuthenticationServiceApplication extends BootMixin(
  ...
  constructor(options: ApplicationConfig = {}) {
    ...
    this.bind(Strategies.Passport.LOCAL_STRATEGY_FACTORY)
      .toProvider(LocalPasswordStrategyFactoryProvider);

    this.bind(Strategies.Passport.BEARER_STRATEGY_FACTORY)
    ...
    // this.bind(AuthServiceBindings.JWT_SIGNER).toProvider(JWTAsymmetricSignerProvider);
